...
std::tuple<Options, bool> ParseCmdLine(int argc, char** argv) {
    ...
    desc.add_options()("cc",
                       value<std::string>(&options.path_to_compile_commands)
                           ->default_value("."),
                       "Directory of the `compile_commands.json` file");
    desc.add_options()("source,s", value<std::string>(&options.source_name),
                       "Source file");
    desc.add_options()("file,f", value<std::string>(&options.file_name),
                       "File to analyze, if empty, equals to 'source file'");
    desc.add_options()(
        "diag,d", value<bool>(&options.display_diagnostic)->default_value(true),
        "Display diagnostics");
    desc.add_options()(
        "exclude-pch",
        value<int>(&options.exclude_declarations_from_pch)->default_value(0),
        "Exclude declarations from precompiled headers (PCH)");
    desc.add_options()("line", value<unsigned>(&options.line),
                       "Line in the file");
    desc.add_options()("col", value<unsigned>(&options.column),
                       "Column on the line");
